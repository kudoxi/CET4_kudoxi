...
class Base extends Frontend
{
    ...
    public function _initialize()
    {
		...
		$this->is_login = 1;
		if($ahan_username == ""){
			Session::set("ahan_username","vistor");
			$ahan_username = "vistor";
		}
		//获取语言分类
        ...
        // 	$this->view->assign("alllanguagelist", $alllanguagelist);
        // }else{
        // 	$this->error("请告诉我你的名字",url("kudoxi/index/index"));
        // }
    }
    	...
    	if($this->request->isPost()){
    		...
    		if(!empty($url_arr)){
    			...
    			if($success){
    				...
    				if($table == "record"){
						$url = url("kudoxi/record/record",array("lid"=>$lid));
					}elseif($table == "vocabulary"){
						$url = url("kudoxi/vocabulary/vocabulary",array("lid"=>$lid));
					}elseif($table == "listen"){
						$url = url("kudoxi/listen/listen",array("lid"=>$lid));
					}
    ...
    public function do_upload(){
	    ...
	    if ($info) { 
	    	//返回文件路径
	    	$filename = \think\Config::get("LOCAHOST2").DS."kudoxi".DS."public".DS. 'uploads'.DS.$info->getSaveName();
	      	return $filename;
	    } else { 
	      	//上传失败获取错误信息 
